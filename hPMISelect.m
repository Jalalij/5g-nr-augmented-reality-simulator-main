%hPMISelect Select precoder matrix indicator
%   [PMI,SINR,SUBBANDIDX] = hPMISelect(NLAYERS, HEST, NOISEEST, BANDSIZE)
...
function [pmi,sinr,subbandIndices] = hPMISelect(nlayers, hest, noiseest, bandSize)
    ...
    if ~isempty(subs) && noiseest ~= 0      
        ...
        for tpmi = 0:maxTPMI
            ...
            for i = 1:length(subs)
                scNo = subs(i,1);
                symbNo = subs(i,2);
                sinr(scNo,symbNo,tpmi+1) = hPrecodedSINR(H(:,:,scNo,symbNo),sigma,W); % Perform codebook selection using LMMSE SINR metric
            end
        end
        
        [sinrBands,subbandIndices] = hSINRPerSubband(sinr, bandSize);
        [~,pmi]  = max(sinrBands,[],2);
