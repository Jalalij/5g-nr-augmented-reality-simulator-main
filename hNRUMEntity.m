...
% Copyright 2020 The MathWorks, Inc.
    ...
    properties (Access = private)
        ...
        TxHeaderBuffer
        % TxTTLBuffer Buffer to store the associated TTL of the data stored
        % in TxBuffer
        TxTTLBuffer
        % TxPDBBuffer Buffer to store the associated PDB of the data stored
        % in TxBuffer
        TxPDBBuffer
        % TxFrameTypeBuffer Buffer to store the associated frameType of the
        % data stored in TxBuffer, 0: no video frame, 1: P-frame, 2 I-frame
        TxFrameTypeBuffer
        % TxPortionOfFrame Buffer to store how much of the frame corresponds to the
        % data stored in TxBuffer
        TxPortionOfFrame
        % TxAppID Buffer to store the associated AppID of the data stored
        % in TxBuffer
        TxAppIDBuffer
    ...
    methods (Access = public)
        function obj = hNRUMEntity(config)
            ...
            obj.TxTTLBuffer = zeros(obj.MaxTxBufferSDUs, 1);
            obj.TxPDBBuffer = zeros(obj.MaxTxBufferSDUs, 1);
            obj.TxPortionOfFrame = zeros(obj.MaxTxBufferSDUs, 1);
            obj.TxFrameTypeBuffer = zeros(obj.MaxTxBufferSDUs, 1);
            obj.TxAppIDBuffer = zeros(obj.MaxTxBufferSDUs, 1);
            ...
            if (obj.EntityType == 0) || (obj.EntityType == 2)
                ...
                obj.TxNext = floor(randi([0 obj.TotalTxSeqNum-1])/10)*10; % the floor operator is to make sure that the Tx-Side can sync to the first sequence number
            end
        ...
        function enqueueSDU(obj, rlcSDU, TTL, PDB, frameTypeIndicator, portionOfFrameTransmitted)
            ...
            obj.TxTTLBuffer(sduEnqueueIdx) = TTL;
            obj.TxPDBBuffer(sduEnqueueIdx) = PDB;
            obj.TxPortionOfFrame(sduEnqueueIdx) = portionOfFrameTransmitted;
            obj.TxFrameTypeBuffer(sduEnqueueIdx) = frameTypeIndicator;
            
            sdapHeader = rlcSDU(4);
            sdapHeader = int2bit(sdapHeader,8);
            AppID = bit2int(sdapHeader(3:end),6); % appID is identificator for QoS flow -> needs to be available at MAC-layer
            obj.TxAppIDBuffer(sduEnqueueIdx) = AppID;
        ...
        function dequeueSDU(obj, header)
            ...
            %dequeueSDU Dequeue the SDU identified by a certain PDCP header
            headerSize = numel(header); %[Byte]

            if mod(obj.TxBufferFront + obj.NumTxBufferSDUs, obj.MaxTxBufferSDUs) == 0
                lastBufferedElementIdx = numel(obj.TxBuffer);
            else
                lastBufferedElementIdx = mod(obj.TxBufferFront + obj.NumTxBufferSDUs, obj.MaxTxBufferSDUs);
            end

            firstBufferedElementIdx = obj.TxBufferFront + 1;

            % loop through all elements and find the header
            sduDequeIdx = [];
            if lastBufferedElementIdx >= firstBufferedElementIdx
                for i=firstBufferedElementIdx:lastBufferedElementIdx
                    if header == obj.TxBuffer{i}(1:headerSize)
                        sduDequeIdx = i;
                        break
                    end
                end
            else
                for i=1:lastBufferedElementIdx
                    if header == obj.TxBuffer{i}(1:headerSize)
                        sduDequeIdx = i;
                        break
                    end
                end
                for i=firstBufferedElementIdx:numel(obj.TxBuffer)
                    if header == obj.TxBuffer{i}(1:headerSize)
                        sduDequeIdx = i;
                        break
                    end
            ...
            if isempty(sduDequeIdx)
                % element not found
                return
            end

            dequeuedSDU = obj.TxBuffer{sduDequeIdx};

            if lastBufferedElementIdx>=firstBufferedElementIdx % buffer does not go over array end
                % move all array elements down by one
                obj.TxBuffer(sduDequeIdx:lastBufferedElementIdx-1) = obj.TxBuffer(sduDequeIdx+1:lastBufferedElementIdx);
                obj.TxHeaderBuffer(sduDequeIdx:lastBufferedElementIdx-1) = obj.TxHeaderBuffer(sduDequeIdx+1:lastBufferedElementIdx);
                obj.TxTTLBuffer(sduDequeIdx:lastBufferedElementIdx-1) = obj.TxTTLBuffer(sduDequeIdx+1:lastBufferedElementIdx);
                obj.TxPDBBuffer(sduDequeIdx:lastBufferedElementIdx-1) = obj.TxPDBBuffer(sduDequeIdx+1:lastBufferedElementIdx);
                obj.TxPortionOfFrame(sduDequeIdx:lastBufferedElementIdx-1) = obj.TxPortionOfFrame(sduDequeIdx+1:lastBufferedElementIdx);
                obj.TxFrameTypeBuffer(sduDequeIdx:lastBufferedElementIdx-1) = obj.TxFrameTypeBuffer(sduDequeIdx+1:lastBufferedElementIdx);
                obj.TxAppIDBuffer(sduDequeIdx:lastBufferedElementIdx-1) = obj.TxAppIDBuffer(sduDequeIdx+1:lastBufferedElementIdx);
            else
                % move all array elements down by one
                obj.TxBuffer(sduDequeIdx:end-1) = obj.TxBuffer(sduDequeIdx+1:end);
                obj.TxHeaderBuffer(sduDequeIdx:end-1) = obj.TxHeaderBuffer(sduDequeIdx+1:end);
                obj.TxTTLBuffer(sduDequeIdx:end-1) = obj.TxTTLBuffer(sduDequeIdx+1:end);
                obj.TxPDBBuffer(sduDequeIdx:end-1) = obj.TxPDBBuffer(sduDequeIdx+1:end);
                obj.TxPortionOfFrame(sduDequeIdx:end-1) = obj.TxPortionOfFrame(sduDequeIdx+1:end);
                obj.TxFrameTypeBuffer(sduDequeIdx:end-1) = obj.TxFrameTypeBuffer(sduDequeIdx+1:end);
                obj.TxAppIDBuffer(sduDequeIdx:end-1) = obj.TxAppIDBuffer(sduDequeIdx+1:end);
                % move first element to the end
                obj.TxBuffer(end) = obj.TxBuffer(1);
                obj.TxHeaderBuffer(end) = obj.TxHeaderBuffer(1);
                obj.TxTTLBuffer(end) = obj.TxTTLBuffer(1);
                obj.TxPDBBuffer(end) = obj.TxPDBBuffer(1);
                obj.TxPortionOfFrame(end) = obj.TxPortionOfFrame(1);
                obj.TxFrameTypeBuffer(end) = obj.TxFrameTypeBuffer(1);
                obj.TxAppIDBuffer(end) = obj.TxAppIDBuffer(1);
                % move all elements at the start of the array down by one
                obj.TxBuffer(1:lastBufferedElementIdx-1) = obj.TxBuffer(2:lastBufferedElementIdx);
                obj.TxHeaderBuffer(1:lastBufferedElementIdx-1) = obj.TxHeaderBuffer(2:lastBufferedElementIdx);
                obj.TxTTLBuffer(1:lastBufferedElementIdx-1) = obj.TxTTLBuffer(2:lastBufferedElementIdx);
                obj.TxPDBBuffer(1:lastBufferedElementIdx-1) = obj.TxPDBBuffer(2:lastBufferedElementIdx);
                obj.TxPortionOfFrame(1:lastBufferedElementIdx-1) = obj.TxPortionOfFrame(2:lastBufferedElementIdx);
                obj.TxFrameTypeBuffer(1:lastBufferedElementIdx-1) = obj.TxFrameTypeBuffer(2:lastBufferedElementIdx);
                obj.TxAppIDBuffer(1:lastBufferedElementIdx-1) = obj.TxAppIDBuffer(2:lastBufferedElementIdx);
            end
            obj.NumTxBufferSDUs = obj.NumTxBufferSDUs - 1;
            if (sduDequeIdx==obj.TxBufferFront+1) && (obj.TxSegmentOffset)
                % if part of the SDU was already transmitted
                dequedRlcPDULength = length(obj.TxHeaderBuffer{obj.TxBufferFront + 1}) + ...
                    length(obj.TxBuffer{obj.TxBufferFront + 1}) - obj.TxSegmentOffset;
                obj.TxSegmentOffset = 0; % for the case that the PDU was segmented reset the indicator
                obj.TxNext = mod(obj.TxNext + 1, obj.TotalTxSeqNum);
            else
                dequedRlcPDULength = length(dequeuedSDU);
            end
            ...
            if obj.NumTxBufferSDUs == 0
                ...
                obj.RequiredGrantLength = obj.RequiredGrantLength - ...
                    getMACHeaderLength(obj, dequedRlcPDULength) - dequedRlcPDULength;
            end
            % Send the updated RLC buffer status report to MAC layer
            obj.TxBufferStatusFcn(getBufferStatus(obj));
        end
        ...
        function rlcPDUSet = sendPDU(obj, bytesGranted, remainingTBS)
            ...
            while (numBytesFilled < bytesGranted) && ...
                ...
                dataType = obj.TxFrameTypeBuffer(obj.TxBufferFront + 1);
                ...
                if ((numBytesFilled + pduLength + macHeaderLength) > bytesGranted) && ...
                    ...
                    minGrantLength = pduHeaderLength + macHeaderLength + 1;
                    ...
                    if dataType ~= 0
                        obj.StatTxVideoFrameBytes = obj.StatTxVideoFrameBytes + numel(segmentedPDU);
                        obj.StatTxVideoFrameTypes = [obj.StatTxVideoFrameTypes; dataType];
                        obj.StatTxPortionOfFrame = [obj.StatTxPortionOfFrame; obj.TxPortionOfFrame(obj.TxBufferFront + 1)];
                    end
                    obj.StatTxTTL = [obj.StatTxTTL; obj.TxTTLBuffer(obj.TxBufferFront + 1)];
                    ...
                    if dataType ~= 0
                        obj.StatTxVideoFrameBytes = obj.StatTxVideoFrameBytes + numel(normalPDU);
                        obj.StatTxVideoFrameTypes = [obj.StatTxVideoFrameTypes; dataType];
                        obj.StatTxPortionOfFrame = [obj.StatTxPortionOfFrame; obj.TxPortionOfFrame(obj.TxBufferFront + 1)];
                    end
                    obj.StatTxTTL = [obj.StatTxTTL; obj.TxTTLBuffer(obj.TxBufferFront + 1)];
        ...
        function bufferStatusReport = getBufferStatus(obj)
            ...
            % return if there is nothing to dequeue
            if obj.NumTxBufferSDUs == 0
                bufferStatusReport.BufferTTL = [];
                bufferStatusReport.BufferPDB = [];
                bufferStatusReport.BufferFrameType = [];
                bufferStatusReport.BufferAppID = [];
                bufferStatusReport.PortionOfFrame = [];
            else
                if mod(obj.TxBufferFront + obj.NumTxBufferSDUs, obj.MaxTxBufferSDUs) == 0
                    lastBufferedElementIdx = numel(obj.TxBuffer);
                else
                ...
                firstBufferedElementIdx = obj.TxBufferFront + 1;
                if lastBufferedElementIdx >= firstBufferedElementIdx
                    bufferStatusReport.BufferTTL = obj.TxTTLBuffer(firstBufferedElementIdx:lastBufferedElementIdx);
                    bufferStatusReport.BufferPDB = obj.TxPDBBuffer(firstBufferedElementIdx:lastBufferedElementIdx);
                    bufferStatusReport.BufferFrameType = obj.TxFrameTypeBuffer(firstBufferedElementIdx:lastBufferedElementIdx);
                    bufferStatusReport.PortionOfFrame = obj.TxPortionOfFrame(firstBufferedElementIdx:lastBufferedElementIdx);
                    bufferStatusReport.BufferAppID = obj.TxAppIDBuffer(firstBufferedElementIdx:lastBufferedElementIdx);
                else
                    bufferStatusReport.BufferTTL = [obj.TxTTLBuffer(firstBufferedElementIdx:end); obj.TxTTLBuffer(1:lastBufferedElementIdx)];
                    bufferStatusReport.BufferPDB = [obj.TxPDBBuffer(firstBufferedElementIdx:end); obj.TxPDBBuffer(1:lastBufferedElementIdx)];
                    bufferStatusReport.BufferFrameType = [obj.TxFrameTypeBuffer(firstBufferedElementIdx:end); obj.TxFrameTypeBuffer(1:lastBufferedElementIdx)];
                    bufferStatusReport.PortionOfFrame = [obj.TxPortionOfFrame(firstBufferedElementIdx:end); obj.TxPortionOfFrame(1:lastBufferedElementIdx)];
                    bufferStatusReport.BufferAppID = [obj.TxAppIDBuffer(firstBufferedElementIdx:end); obj.TxAppIDBuffer(1:lastBufferedElementIdx)];
                end
            end
        end
        ...
        function sdu = receivePDU(obj, packet)
            ...
            if decodedPDU.SegmentationInfo == 0
                ...
                if isnan(obj.RxNextHighest)
                    obj.RxNextReassembly = floor(decodedPDU.SequenceNumber/10)*10;
                    obj.RxNextHighest = floor(decodedPDU.SequenceNumber/10)*10;
                end
        ...
        function handleTimerTrigger(obj)
            %handleTimerTrigger Process 1 microsecond timer trigger

            if obj.ReassemblyTimeLeft > 0
                % Decrement the time left for reassembly timer expiry
                obj.ReassemblyTimeLeft = obj.ReassemblyTimeLeft - 0.001;
            ...
            obj.TxTTLBuffer = obj.TxTTLBuffer - 0.001;

            % update the buffer for delay-aware scheduling
            obj.TxBufferStatusFcn(getBufferStatus(obj));
        end
    end
