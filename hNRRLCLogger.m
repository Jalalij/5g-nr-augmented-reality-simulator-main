classdef hNRRLCLogger < handle
    ...
    properties (Access = private, Constant)
        ...
        RLCStatsTitles = {'RNTI', 'LCID', 'TxDataPDU', 'TxDataBytes', ...
            ...
            'TimerReassemblyTimedOut', 'TimerStatusProhibitTimedOut', 'TxVideoFrameBytes', 'TxRemainingTTL', 'TxVideoFrameType', 'TxPortionOfVideoFrame'};
    end
    ...
    methods (Access = public)
        ...
        function logCellRLCStats(obj)
            ...
            for ueIdx = 1:obj.NumUEs
                ...
                if ~isempty(obj.PrevUERLCStats{ueIdx})
                    numRows = numel(stats(:,1));
                    for rowIdx = 1:numRows
                        obj.UERLCStats{ueIdx}(rowIdx, 3:end-3) = num2cell(cell2mat(stats(rowIdx, 3:end-3)) - cell2mat(obj.PrevUERLCStats{ueIdx}(rowIdx, 3:end-3)));

                        prevNumEntries = numel(cell2mat(obj.PrevUERLCStats{ueIdx}(rowIdx, end-2)));
                        numEntries = numel(cell2mat(stats(rowIdx, end-2)));
                        if numEntries>prevNumEntries
                            ttlStats = cell2mat(stats(rowIdx, end-2));
                            obj.UERLCStats{ueIdx}(rowIdx, end-2) = {ttlStats(prevNumEntries+1:numEntries)};
                        else
                            obj.UERLCStats{ueIdx}(rowIdx, end-2) = cell(1,1);
                        end

                        prevNumEntries = numel(cell2mat(obj.PrevUERLCStats{ueIdx}(rowIdx, end-1)));
                        numEntries = numel(cell2mat(stats(rowIdx, end-1)));
                        if numEntries>prevNumEntries
                            videoFrameTypeStats = cell2mat(stats(rowIdx, end-1));
                            obj.UERLCStats{ueIdx}(rowIdx, end-1) = {videoFrameTypeStats(prevNumEntries+1:numEntries)};
                        else
                            obj.UERLCStats{ueIdx}(rowIdx, end-1) = cell(1,1);
                        end

                        prevNumEntries = numel(cell2mat(obj.PrevUERLCStats{ueIdx}(rowIdx, end)));
                        numEntries = numel(cell2mat(stats(rowIdx, end)));
                        if numEntries>prevNumEntries
                            videoTxPortionStats = cell2mat(stats(rowIdx, end));
                            obj.UERLCStats{ueIdx}(rowIdx, end) = {videoTxPortionStats(prevNumEntries+1:numEntries)};
                        else
                            obj.UERLCStats{ueIdx}(rowIdx, end) = cell(1,1);
                        end
                    end
                ...
                if ~isempty(obj.PrevGNBRLCStats{ueIdx})
                    numRows = numel(stats(:,1));
                    for rowIdx = 1:numRows
                        obj.GNBRLCStats{ueIdx}(rowIdx, 3:end-3) = num2cell(cell2mat(stats(rowIdx, 3:end-3)) - cell2mat(obj.PrevGNBRLCStats{ueIdx}(rowIdx, 3:end-3)));
                        
                        prevNumEntries = numel(cell2mat(obj.PrevGNBRLCStats{ueIdx}(rowIdx, end-2)));
                        numEntries = numel(cell2mat(stats(rowIdx, end-2)));
                        if numEntries>prevNumEntries
                            ttlStats = cell2mat(stats(rowIdx, end-2));
                            obj.GNBRLCStats{ueIdx}(rowIdx, end-2) = {ttlStats(prevNumEntries+1:numEntries)};
                        else
                            obj.GNBRLCStats{ueIdx}(rowIdx, end-2) = cell(1,1);
                        end

                        prevNumEntries = numel(cell2mat(obj.PrevGNBRLCStats{ueIdx}(rowIdx, end-1)));
                        numEntries = numel(cell2mat(stats(rowIdx, end-1)));
                        if numEntries>prevNumEntries
                            videoFrameTypeStats = cell2mat(stats(rowIdx, end-1));
                            obj.GNBRLCStats{ueIdx}(rowIdx, end-1) = {videoFrameTypeStats(prevNumEntries+1:numEntries)};
                        else
                            obj.GNBRLCStats{ueIdx}(rowIdx, end-1) = cell(1,1);
                        end
                        
                        prevNumEntries = numel(cell2mat(obj.PrevGNBRLCStats{ueIdx}(rowIdx, end)));
                        numEntries = numel(cell2mat(stats(rowIdx, end)));
                        if numEntries>prevNumEntries
                            videoTxPortionStats = cell2mat(stats(rowIdx, end));
                            obj.GNBRLCStats{ueIdx}(rowIdx, end) = {videoTxPortionStats(prevNumEntries+1:numEntries)};
                        else
                            obj.GNBRLCStats{ueIdx}(rowIdx, end) = cell(1,1);
                        end
                    end
        ...
        function logRLCStats(obj, ueRLCStats, gNBRLCStats)
            ...
            currgNBStats = [];
            for ueIdx = 1:obj.NumUEs
                currUEStats = [currUEStats; ueRLCStats{ueIdx}];
                currgNBStats = [currgNBStats; gNBRLCStats{ueIdx}];
            end
            ...
            obj.FinalUERLCStats(:,3:end-3) = cell2mat(currUEStats(:,3:end-3)) + obj.FinalUERLCStats(:,3:end-3);
            obj.FinalgNBRLCStats(:,3:end-3) = cell2mat(currgNBStats(:,3:end-3)) + obj.FinalgNBRLCStats(:,3:end-3);
            % Add column titles for the current slot statistics
            obj.RLCStatsLog{logIndex, obj.ColumnIndexMap('UE RLC statistics')} = vertcat(obj.RLCStatsTitles, currUEStats);
            obj.RLCStatsLog{logIndex,obj.ColumnIndexMap('gNB RLC statistics')} = vertcat(obj.RLCStatsTitles, currgNBStats);
        end
