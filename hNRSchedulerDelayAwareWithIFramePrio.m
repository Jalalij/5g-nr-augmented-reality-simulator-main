classdef hNRSchedulerDelayAwareWithIFramePrio < hNRScheduler
    ...
    methods
        function obj = hNRSchedulerDelayAwareWithIFramePrio(simParameters)
            ...
            for i = 1:length(schedulerInput.eligibleUEs)
                ...
                if(bufferStatus > 0) % Check if UE has any data pending
                    if schedulerInput.LinkDir == 0 % DL
                        frameType = zeros(size(schedulerInput.lchFrameType{i})); % for every packet in the RLC buffer, 0: no video frame, 1: P-frame, 2: I-frame
...
%                             frameType(j) = schedulerInput.lchFrameType{i}(j);
...
%                         end
                        ...
                        for j=1:numel(schedulerInput.lchTTL{i})
                            priority(j) = getSchedulingPriority(obj, schedulerInput.lchAppID{i}(j), schedulerInput.eligibleUEs(i));
                            if ceil(schedulerInput.lchTTL{i}(j)/obj.SlotDuration) <= 3 && ceil(schedulerInput.lchTTL{i}(j)/obj.SlotDuration) > 0 % in the last two slots prioritize
                                delayBudgetFactor(j) = (4-ceil(schedulerInput.lchTTL{i}(j)/obj.SlotDuration))*1e12;
                            else
                                delayBudgetFactor(j) = 1;
                            end
                        end
                        ...
                        pfWeightageFull = (100-priority).*achievableDataRate/pastDataRate.*delayBudgetFactor.*((frameType== 2)*9+1);
