classdef hNRUEPhy < hNRPhyInterface
    ...
    properties (Access = private)
        ...
        %DLCarrierFreq Downlink carrier frequency
        DLCarrierFreq = 2.1e9;

        %ShadowFadingLoss Loss caused by shadow fading in dB
        ShadowFadingLoss = 0;

        %LOS Indicator for a LOS connection
        LOS = false;

        %UEIndoorDistance Distance of the UE to the outer wall
        UEIndoorDistance = 0;
...
%                 obj.TxAntPanel = param.RxAntPanel;
%             end
...
%             end
            ...
            if isfield(param, 'DLCarrierFreq')
                obj.DLCarrierFreq = param.DLCarrierFreq;
            end
    ...
    methods (Access = private)
        ...
        function rxWaveform = applyChannelModel(obj, pktInfo, currentTime)
            ...
            if isempty(obj.PLConfig)
                ...
                pathLoss = nrPathLossWithIndoorPropagation(obj.PLConfig,obj.DLCarrierFreq,obj.LOS,obj.UEIndoorDistance,pktInfo.Position.',obj.Node.Position.');
                rxWaveform = db2mag(-pathLoss-obj.ShadowFadingLoss)*rxWaveform;
            end
            ...
            for idx=1:length(csirsInfoList)
                ...
                if length(csirsInfo.RowNumber) == 1
                    ...
                    if ~isempty(csirsRefInd)
                        ...
                        if obj.NumRxAnts > 1
                            rank = hRISelectSystem(carrier, csirsInfo, obj.CSIReportConfig, Hest, nVar);
