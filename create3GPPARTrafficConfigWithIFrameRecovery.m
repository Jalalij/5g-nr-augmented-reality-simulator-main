function [appTable, sdapTable, pdcpTable, rlcTable] = create3GPPARTrafficConfigWithIFrameRecovery(simParamters)
...
for i = 1:numUEs
    % App config
    App_Name_full(end+1,1) = "ARvideoTraffic"; % DL video App
    App_RNTI_full(end+1,1) = RNTI(i);
    App_ID_full(end+1,1) = 1;
    App_ProtocolOverhead_full(end+1,1) = 40;
    App_HostDevice_full(end+1,1) = 0;
    App_Parameter1(end+1,1) = "multiStreamGOPDL_1";
    App_Parameter2(end+1,1) = "SingleEyeBuffer";
    App_Parameter3(end+1,1) = "2";
    App_Parameter4(end+1,1) = "10000";
    ...
    PDCP_DataTimeToLive_full(end+1,1) = 10; % time to live until next frame is generated
    ...
    RLC_MaxTxBufferSDUs_full(end+1,1) = 128; % if buffer size is exceeded packet is dropped
