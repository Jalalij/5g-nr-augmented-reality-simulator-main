classdef hNRSchedulerDelayAware < hNRScheduler
    %hNRSchedulerDelayAware Implements delay aware scheduler 
    % described in K. Koutlia et. al., System Analysis of QoS schedulers for
    % XR traffic in 5G NR, 2023
    ...
    properties
        ...
        DelayFactorWeight = 10;

        % AppIDto5QIMapping Specifies the mapping between AppID present in
        % the SDAP header and the 5QI value, 
        % - column index represents RNTI
        % - row index represents AppID
        % - content of the array specifies 5QI value
        AppIDto5QIMapping
    ...
    methods
        function obj = hNRSchedulerDelayAware(simParameters)
            ...
            if isfield(simParameters, 'DelayFactorWeight')
                obj.DelayFactorWeight = simParameters.DelayFactorWeight;
            end

            if isfield(simParameters, 'SDAPconfig')
                obj.AppIDto5QIMapping = simParameters.SDAPconfig.AppIDto5QIMapping;
            end
        ...
        function [selectedUE, mcsIndex] = runSchedulingStrategy(obj, schedulerInput)
            ...
            for i = 1:length(schedulerInput.eligibleUEs)
                ...
                if(bufferStatus > 0) % Check if UE has any data pending
                    if schedulerInput.LinkDir == 0 % DL
                        priority = zeros(size(schedulerInput.lchAppID{i})); % for every packet in the RLC buffer
                        % K. Koutlia et.al, "System Analysis of QoS schedulers for XR traffic in 5G NR"
%                         delayBudgetFactor = zeros(size(schedulerInput.lchTTL{i})); % for every packet in the RLC buffer
%                         for j=1:numel(schedulerInput.lchTTL{i})
%                             priority(j) = getSchedulingPriority(obj, schedulerInput.lchAppID{i}(j), schedulerInput.eligibleUEs(i));
%                             delayBudgetFactor(j) = schedulerInput.lchPDB{i}(j)/(schedulerInput.lchTTL{i}(j));
%                         end

                        % schedule UEs depending on remaining TTL
                        %delayBudgetFactor = zeros(size(schedulerInput.lchTTL{i})); % for every packet in the RLC buffer
                        %for j=1:numel(schedulerInput.lchTTL{i})
                        %    priority(j) = getSchedulingPriority(obj, schedulerInput.lchAppID{i}(j), schedulerInput.eligibleUEs(i));
                        %    if ceil(schedulerInput.lchTTL{i}(j)/obj.SlotDuration) > 0
                        %        delayBudgetFactor(j) = (1+obj.DelayFactorWeight/10).^(-ceil(schedulerInput.lchTTL{i}(j)/obj.SlotDuration));
                        %    else
                        %        delayBudgetFactor(j) = 0; % dont schedule the UE if its already too late
                        %    end
                        %end

                        % prioritize UEs in last slots
                        delayBudgetFactor = zeros(size(schedulerInput.lchTTL{i})); % for every packet in the RLC buffer
                        for j=1:numel(schedulerInput.lchTTL{i})
                            priority(j) = getSchedulingPriority(obj, schedulerInput.lchAppID{i}(j), schedulerInput.eligibleUEs(i));
                            if ceil(schedulerInput.lchTTL{i}(j)/obj.SlotDuration) <= 0
                                delayBudgetFactor(j) = -1e15; % dont schedule the UE if its already too late
                            elseif ceil(schedulerInput.lchTTL{i}(j)/obj.SlotDuration) <= obj.DelayFactorWeight
                                delayBudgetFactor(j) = 1e15;
                            else % if there is enough time left do normal PF scheduling
                                delayBudgetFactor(j) = 0;
                            end
                        end
                        ...
                        % Weightage for every packet in the RLC buffer
                        pfWeightageFull = (100-priority).*achievableDataRate/pastDataRate + delayBudgetFactor;
                        pfWeightage = max(pfWeightageFull);
                    else % UL
                        bitsPerSym = schedulerInput.mcsRBG(i, 2); % Accounting for both Modulation & Coding scheme
                        numLayers = schedulerInput.selectedRank(i);
                        achievableDataRate = ((numLayers* schedulerInput.RBGSize * bitsPerSym * 14 * 12)*1000)/ ...
    ...
    methods(Access = private)
        function priority = getSchedulingPriority(obj, appID, RNTI)
            QFI = obj.AppIDto5QIMapping(RNTI,appID);

            switch QFI % https://5g-tools.com/qos-for-5g-nr/
                case 1
                    priority = 20;
                case 2
                    priority = 40;
