...
%                     This field is not applicable when CodebookType is
%                     specified as 'Type2'
...
%        'Type1MultiPanel'. It is an array with the elements in the
%        following order [q1 q2 i12 i131 AmplitudeSet1 i132 AmplitudeSet2]
%        when CodebookType is 'Type2'. The elements are placed in this
%        order according to TS 38.214 Section 5.2.2.2.3
...
%                  2*reportConfig.NumberOfBeams-by-2*nLayers-by-numSubbands.
%                  In this case, the index set for each subband is in the
%                  form [i211 i221 i212 i222], where [i211 i221]
%                  corresponds to co-phasing and subband amplitude values
%                  for layer 1 and [i212 i222] corresponds to co-phasing
%                  and subband amplitude values for layer 2, as defined in
%                  TS 38.214 Section 5.2.2.2.3
%                - When SubbandAmplitude is false,
%                  it is a matrix of size
%                  2*reportConfig.NumberOfBeams-by-nLayers-by-numSubbands.
%                  In this case, the index set for each subband is in the
%                  form of i211 or [i211 i212], where i211 corresponds to
%                  co-phasing values for layer 1 and i212 corresponds to
%                  co-phasing values for layer 2, as defined in
%                  TS 38.214 Section 5.2.2.2.3
...
%   Copyright 2019-2022 The MathWorks, Inc.
    ...
    if ~isCSIRSObjSyntax
        ...
        if isempty(csirsIndSubs_k) || (nVar == 0)
            ...
            PMIInfo.SINRPerREPMI = NaN(reportConfig.NSizeBWP*12,carrier.SymbolsPerSlot,nLayers);
            PMIInfo.W = W;
        else
            sigma = sqrt(nVar);
            ...
            SINRperSubbandperCW = getSubbandSINR(SINRsperRE,CQISubbandInfo); % Corresponds to single codeword
            ...
            SINRsperRBperCW = getSINRperRB(SINRsperRE);
        ...
        if strcmpi(reportConfig.CodebookType,'Type2')
            ...
            if strcmpi(reportConfig.CQIMode,'Subband') && strcmpi(reportConfig.PMIMode,'Wideband')                
                SINRperSubband = getSubbandSINR(sinrPerREPMI,CQISubbandInfo);
            end
            ...
            SINRsperRBperCW = getSINRperRB(sinrPerREPMI);
        else
            ...
            if isfield(reportConfig,'PRGSize') && ~isempty(reportConfig.PRGSize)
                ...
                if strcmpi(reportConfig.CQIMode,'Subband')
                    ...
                    SINRsperRECQI = getSINRperRECQI(PMIInfo.SINRPerRE,struct('i1',PMISet.i1,'i2',randomi2),CQISubbandInfo.SubbandSizes);
                else
                ...
                SINRsperRBperCW = getSINRperRB(SINRsperRECQI);                
            else
    ...
    if ~inputSINRTable
        ...
        for subbandIdx = 1:CQISubbandInfo.NumSubbands
            ...
            k = (subbandStart*12 + 1):(subbandStart + CQISubbandInfo.SubbandSizes(subbandIdx))*12;
            % Compute CQI, effective SINR and estimated BLER
            [CQIForAllSubbands(subbandIdx,:),SINRperSubbandperCW(subbandIdx,:),BLERForAllSubbands(subbandIdx,:)] = ...
                cqiSelect(carrier,pdsch,pdschExt.XOverhead,PMIInfo.SINRPerREPMI(k,:,:),reportConfig.CQITable);
        ...
        if size(SINRperSubbandperCW,1) > 1
            ...
            [wbCQI,wbEffectiveSINR,wbBLER] = cqiSelect(carrier,pdsch,pdschExt.XOverhead,PMIInfo.SINRPerREPMI,reportConfig.CQITable);
...
function SINRsperRECQI = getSINRperRECQI(SINRsperRE,PMISet,subbandSizes)
%   SINRSPERRECQI = getSINRperRECQI(SINRSPERRE,PMISET,SUBBANDSIZES) returns
...
function SINRsperRBperCW = getSINRperRB(SINRsperRECQI)
%   SINRSPERRBPERCW = getSINRperRB(SINRSPERRECQI) returns
...
function SubbandSINRs = getSubbandSINR(SINRsperREPMI,SubbandInfo)
%   SUBBANDSINRS = getSubbandSINR(SINRSPERREPMI,SUBBANDINFO) returns
...
%      SubbandSizes - Size of each subband
    ...
    SubbandSINRs = NaN(SubbandInfo.NumSubbands,size(SINRsperREPMI,3));
    % Consider the starting position of first subband as start of BWP
    subbandStart = 0;
    for SubbandIdx = 1:SubbandInfo.NumSubbands
        sinrTmp = SINRsperREPMI((subbandStart*12 + 1):(subbandStart+ SubbandInfo.SubbandSizes(SubbandIdx))*12,:,:);
        if ~all(isnan(sinrTmp(:)))
            SubbandSINRs(SubbandIdx,:) = squeeze(mean(mean(sinrTmp,'omitnan'),'omitnan'));
        end
...
%   table SINRTABLE for SINR to CQI mapping, and the noise variance NVAR.
    ...
    if isfield(reportConfig,'CQITable')
        reportConfigOut.CQITable = validatestring(reportConfig.CQITable,{'table1','table2','table3'},fcnName,'CQITable field');
    else
        reportConfigOut.CQITable = 'table1';
    end
    ...
    elseif strcmpi(reportConfigOut.CodebookType,'Type1MultiPanel')
...
function [pdsch,pdschExt] = hCSIReferenceResource(carrier,reportConfig,numLayers)
    ...
    pdsch = nrPDSCHConfig;
    pdsch.PRBSet = reportConfig.NStartBWP + (0:reportConfig.NSizeBWP-1);
...
function [cqiIndex,effectiveSINR,transportBLER] = cqiSelect(carrier,pdsch,xOverhead,SINRs,cqiTableName)

    % Load RBIR and AWGN table data
    persistent data;
    if isempty(data)
    ...
    cqiTable = hCQITables(cqiTableName);
    cqiTable = cqiTable(:,2:3);

    % Perform effective SINR mapping using IEEE 802.11 methodology, for
    % each modulation order in the CQI table and for each codeword
    Qms = unique(cqiTable(:,1));
    Qms = Qms(~isnan(Qms));
    Qms1 = Qms;
    nQm = numel(Qms);
    QmSINR = zeros(nQm,ncw);
    SINRs = nrLayerDemap(SINRs);
    for i = 1:nQm
        QmSINR(i,:) = effectiveSINRMapping(repmat(Qms(i),1,ncw),SINRs);
    end

    % Create all combinations of Qm and CQI indices across the codewords,
    % and all combinations of modulation orders corresponding to Qm indices
    comboIdxFn = @(x)nchoosek(1:(x+ncw-1),ncw) - (0:ncw-1);
    QmsIdx = comboIdxFn(nQm);
    Qms = Qms(QmsIdx);
    nCQI = size(cqiTable,1);
    cqiIndex = comboIdxFn(nCQI);
    numCQIIndex = size(cqiIndex,1);
    mods = {'QPSK' '16QAM' '64QAM' '256QAM'}';
    allQms = [2 4 6 8];
    mods = mods(any(allQms == Qms1,1));
    mods = mods(QmsIdx);

    % For each CQI combination, determine the effective SINR and BLER
    [~,pdschIndicesInfo] = nrPDSCHIndices(carrier,pdsch);
    codeBLER = NaN(ncw,numCQIIndex);
    effectiveSINR = NaN(ncw,numCQIIndex);
    C = NaN(ncw,numCQIIndex);
    for i = 1:numCQIIndex
        Qm = cqiTable(cqiIndex(i,:),1)';
        if all(Qm > 0)
            R = cqiTable(cqiIndex(i,:),2);
            targetCodeRates = R / 1024;
            modComboIndex = all(Qms==Qm,2);
            modulations = mods(modComboIndex,:);
            trBlkSizes = nrTBS(modulations,pdsch.NumLayers,numel(pdsch.PRBSet),pdschIndicesInfo.NREPerPRB,targetCodeRates,xOverhead);
            for cwIdx = 1:ncw
                dlschInfo = nrDLSCHInfo(trBlkSizes(cwIdx),targetCodeRates(cwIdx));
                BGN = dlschInfo.BGN;
                Zc = dlschInfo.Zc;
                C(cwIdx,i) = dlschInfo.C;
                t = getAWGNTable(data.awgnTables,BGN,R(cwIdx),Qm(cwIdx),Zc);
                effectiveSINR(cwIdx,i) = QmSINR(Qms1==Qm(cwIdx),cwIdx);
                codeBLER(cwIdx,i) = wireless.internal.L2SM.interpolatePER(effectiveSINR(cwIdx,i),t);
            end
        end
    end
    ...
    % Select the CQI with the largest BLER less than the threshold and
    % select the corresponding effective SINR and BLER
    lastIdx = find(all(transportBLER<=blerThreshold,1),1,'last');
    cqiIndex = max([zeros(1,ncw); cqiIndex(lastIdx,:)-1],[],1);
    effectiveSINR = db2pow(mean(effectiveSINR(:,lastIdx).',1));
    transportBLER = mean(transportBLER(:,lastIdx).',1);

end

function effectiveSINR = effectiveSINRMapping(Qm,SINRs)
    
    alpha = 1.0;
    beta = 1.0;

    nCodewords = numel(Qm);
    effectiveSINR = zeros(1,nCodewords);
    for cwIdx = 1:nCodewords
        effectiveSINR(cwIdx) = wireless.internal.L2SM.calculateEffectiveSINR(SINRs{cwIdx},2^Qm(cwIdx),alpha,beta);
    end
