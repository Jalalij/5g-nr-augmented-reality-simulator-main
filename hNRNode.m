classdef (Abstract) hNRNode < comm_sysmod.internal.wirelessNode
    ...
    properties (Access = public)
        ...
        % SDAPEntity SDAP entities associated with this node. For gNB, it
        % is an array with each row corresponding to a UE
        SDAPEntity

        % PDCPEntities A set of PDCP entities
        % Each entity sends/gets data to one or more RLC Entities. For
    ...
    properties (Access = protected)
        ...
        % PDCPNextInvokeTime Time (in nanoseconds) after which PDCP will get invoked
        PDCPNextInvokeTime = 0;
    ...
    properties (Access = protected, Constant)
        ...
        % MaxDataRadioBearers Maximum number of data radio bearers
        MaxDataRadioBearers = 4;
        % NumRLCStats Number of RLC layer statistics collected
        NumRLCStats = 25;
        % RLCInvocationPeriodicity RLC invocation periodicity in nanoseconds
        RLCInvocationPeriodicity = 1e3;
        %PDCPInvocationPeriodicity RLC invocation periodicity in nanoseconds
        PDCPInvocationPeriodicity = 1e3;
    end
    ...
    methods
        ...
        function addApplication(obj, rnti, appID, app)
            ...
            % helps the application layer in packet generation and
            % transmission
        ...
        function registerCallbackToSourceApplication(obj, rnti, appID, app)
            %registerCallbackToSourceApplication Register an out of band call back from destination back to source

            registerCallback(obj.AppLayer, @app.receiveFeedbackFromRx, rnti, appID);
        end

        function configurePDUSession(obj, rnti, sdapChannelConfig)
            ...
            if macEntity.MACType
                ...
                sdapEntity = hNRSDAPEntity(sdapChannelConfig);
                obj.SDAPEntity = sdapEntity;
            else
                % If it is gNB, then RNTI becomes logical channel set index
                sdapChannelSetIndex = rnti;
                sdapEntity = hNRSDAPEntity(sdapChannelConfig);
                obj.SDAPEntity{sdapChannelSetIndex, 1} = sdapEntity;
            end

            % define the interface to application layer for SDU reception
            appLayer = obj.AppLayer;
            registerAppReceiverFcn(sdapEntity, @appLayer.receivePacket);
        end

        function configureDataRadioBearer(obj, rnti, pdcpChannelConfig)
            
            macEntity = obj.MACEntity;
            % Determine the logical channel index of a UE in the logical
            % channel set
            if macEntity.MACType
                % If it is UE, there will be only one pdcp channel set
                pdcpChannelSetIndex = 1;
            else
                % If it is gNB, then RNTI becomes logical pdcp set index
                pdcpChannelSetIndex = rnti;
            end

            % Check whether the new data radio bearer can be established
            % between the UE and its associated gNB
            drbIdx = find(cellfun(@isempty, obj.PDCPEntities(pdcpChannelSetIndex, :)), 1);
            if isempty(drbIdx)
                error('nr5g:hNRNode:TooManyDataRadioBearers', ...
                    ['Number of logical channels between UE', num2str(rnti), ...
                    ' and its associated gNB must not exceed the configured limit ', num2str(obj.MaxDataRadioBearers)]);
            end

            pdcpEntity = hNRPDCPEntity(pdcpChannelConfig,obj.MACEntity.MACType);
            obj.PDCPEntities{pdcpChannelSetIndex, drbIdx} = pdcpEntity;
           

            % define the interface to SDAP layer for SDU reception
            sdapEntity = getSDAPEntity(obj, rnti);
            registerSDAPReceiverFcn(obj.PDCPEntities{pdcpChannelSetIndex, drbIdx},@sdapEntity.receiveSDU);

            % define interface from SDAP to PDCP layer for SDU transmission
            registerPDCPTransmitterFcn(sdapEntity, @pdcpEntity.transmitSDU, pdcpChannelConfig);
        end
        ...
        function configureLogicalChannel(obj, rnti, rlcChannelConfig)
            ...
            pdcpLayer = getPDCPEntity(obj, rnti, rlcChannelConfig.LogicalChannelID);
            % Register PDCP interface function with the RLC entity
            registerPDCPReceiverFcn(obj.RLCEntities{logicalChannelSetIndex, lchIdx}, @pdcpLayer.receiveSDU);
            rlcLayer = obj.RLCEntities{logicalChannelSetIndex, lchIdx};
            registerRLCTransmitterFcn(pdcpLayer, @rlcLayer.enqueueSDU, rlcChannelConfig);
            registerRLCDequeFcn(pdcpLayer, @rlcLayer.dequeueSDU);
        ...
        function transmitSDAPSDU(obj, sdapSDUInfo, ~)
            if obj.MACEntity.MACType
                sdapEntity = getSDAPEntity(obj, obj.MACEntity.RNTI);
            else
                % gnb has numberOfUE SDAP entities -> destination ID
                % determines which SDAP entity to choose
                sdapEntity = getSDAPEntity(obj, sdapSDUInfo.DestinationID);
            end

            if ~isempty(sdapEntity)
                % Send the received SDAP SDU to the corresponding SDAP entity
                transmitSDU(sdapEntity, sdapSDUInfo.Data, sdapSDUInfo.AppID, sdapSDUInfo.Frame);
            else
        ...
        function rlcStats = getRLCStatistics(obj, rnti)
            ...
            rlcStatsList = cell(obj.MaxLogicalChannels, obj.NumRLCStats);
    ...
    methods(Access = private)
        function nextInvokeTime = runLayers(obj, currentTime, packets)
            ...
            nextAppTime = run(obj.AppLayer, currentTime, @obj.transmitSDAPSDU);
            % Update PDCP timer for every 1ms
            nextPDCPTime = updatePDCPTimer(obj, currentTime);
            ...
            nextInvokeTime = min([nextAppTime nextPDCPTime nextRLCTime nextMACTime nextPHYTime]) * 1e-9;
        end
        ...
        function nextRLCTime = updateRLCTimer(obj, currentTime)
            ...
            %{
            if currentTime < obj.RLCNextInvokeTime
                nextRLCTime = obj.RLCNextInvokeTime;
                return;
            end
             %}
            ...
            obj.RLCNextInvokeTime = obj.RLCInvocationPeriodicity;
        end
        
        function nextPDCPTime = updatePDCPTimer(obj, currentTime)
            if currentTime < obj.PDCPNextInvokeTime
                nextPDCPTime = obj.PDCPNextInvokeTime;
                return;
            end          
            %updatePDCPTimer Advance timer in all PDCP entities by 1 ms
            for uePDCPEntityIdx = 1:size(obj.PDCPEntities, 1)
                for pdcpEntityIdx = 1:obj.MaxDataRadioBearers
                    pdcpEntity = obj.PDCPEntities{uePDCPEntityIdx, pdcpEntityIdx};
                    % Check if the PDCP entity exists
                    if isempty(pdcpEntity)
                        continue;
                    end
                    handleTimerTrigger(pdcpEntity, currentTime);
                end
            end
            % Update the PDCP next invoke time
            nextPDCPTime = currentTime + obj.PDCPInvocationPeriodicity;
            obj.PDCPNextInvokeTime = nextPDCPTime;
        end
        
        function sdapEntity = getSDAPEntity(obj, rnti)
            if obj.MACEntity.MACType % UE
                sdapEntity = obj.SDAPEntity;
            else % gNB
                % If it is gNB then RNTI becomes row index
                rowIdx = rnti;
                sdapEntity = obj.SDAPEntity{rowIdx, 1};
            end
        end

        function pdcpEntity = getPDCPEntity(obj, rnti, lcid)
            %getRLCEntity Return the PDCP entity which the lcid is
            %associated with
            ...
            pdcpEntity = [];
            for colIdx = 1:obj.MaxDataRadioBearers
                entity = obj.PDCPEntities{rowIdx, colIdx};
                % Check the existence of RLC entity before accessing its
                % data
                if isempty(entity)
                    continue;
                end
                if any(entity.LogicalChannelIDs == lcid)
                    pdcpEntity = entity;
                    break;
                end
            end
