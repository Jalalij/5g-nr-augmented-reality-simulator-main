classdef networkTrafficULWebBrowsing < comm_sysmod.internal.ConfigBaseHandle
    ...
    methods
        function obj = networkTrafficULWebBrowsing(varargin)
            obj@comm_sysmod.internal.ConfigBaseHandle(varargin{:});

            obj.pHTTPRequestCountdown = rand(1,1)*generateRandomReadingTime(obj); % random number to simulate how much time has already passed before simulation start
            obj.pTCPAckCountdown = obj.pHTTPRequestCountdown + generateRandomParsingTime(obj);
        end
        ...
        function [dt, packetSize, varargout] = generate(obj, varargin)
            ...
            if obj.pNextInvokeTime <= 0
                ...
                if ~obj.pHTTPRequestSize && ~obj.pTCPAckSize && obj.pHTTPRequestCountdown<=0
                    ...
                    obj.pHTTPRequestCountdown =  generateRandomReadingTime(obj);
                end
                if ~obj.pTCPAckSize && obj.pTCPAckCountdown<=0
                ...
                switch obj.pDataType(1)
                    case 0 % payload type 0 means no packet to output
                        ...
                        varargout{2} = 0;
                    case 1
                        [dt, packetSize, obj.pHTTPRequestSize] = obj.transmitSegment(obj.pHTTPRequestSize);
                        varargout{2} = 1;
                    case 2
                        [dt, packetSize, obj.pTCPAckSize] = obj.transmitSegment(obj.pTCPAckSize);
                        varargout{2} = 2;
                end
                ...
                obj.pHTTPRequestCountdown = obj.pHTTPRequestCountdown - dt;
                obj.pHTTPRequestCountdown = round(obj.pHTTPRequestCountdown*1e6)/1e6; % Limiting dt to nanoseconds accuracy
                obj.pTCPAckCountdown = obj.pTCPAckCountdown - dt;
                obj.pTCPAckCountdown = round(obj.pTCPAckCountdown*1e6)/1e6; % Limiting dt to nanoseconds accuracy
    ...
    methods(Access = private)
        ...
        function generateNewPayload(obj, dataToGenerate)
            ...
            if dataToGenerate == 1
                % Generate HTTP request size
                newPayloadSize = 350;
                obj.pHTTPRequestSize = newPayloadSize;
            else
                % Generate TCP Ack size
                newPayloadSize = 40;
                obj.pTCPAckSize = newPayloadSize;
            end
        ...
        function dt = getNextInvokeTime(obj)
            if numel(obj.pJitters) == 1
                obj.pJitters(end+1) = min(obj.pHTTPRequestCountdown, obj.pTCPAckCountdown);
                obj.pLatencies(end+1) = obj.pLatencies(end)+min(obj.pHTTPRequestCountdown, obj.pTCPAckCountdown);
