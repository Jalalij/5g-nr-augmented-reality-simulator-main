classdef (Abstract) hNRScheduler < handle
    ...
    properties (SetAccess = protected, GetAccess = public)
        ...
        %BufferStatusDLPerUETemp Temporary buffer status which is updated during
        % scheduling
        BufferStatusDLPerUETemp
        ...
        %BufferStatusULPerUETemp Temporary buffer status which is updated during
        % scheduling
        BufferStatusULPerUETemp
    ...
    methods
        function obj = hNRScheduler(param)
            ...
            obj.BufferStatusDLPerUETemp = zeros(numUEs, 1);
            obj.BufferStatusULPerUETemp = zeros(numUEs, 1);
        ...
        function resourceAssignments = runDLScheduler(obj, currentTimeInfo, lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion)
            ...
            for i=1:length(slotsToBeScheduled)
                ...
                slotDLGrants = scheduleDLResourcesSlot(obj, slotsToBeScheduled(i), lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion);
        ...
        function updateLCBufferStatusDL(obj, lcBufferStatus)
            ...
            obj.BufferStatusDLPerUETemp(lcBufferStatus.RNTI) = sum(obj.BufferStatusDL(lcBufferStatus.RNTI, :), 2);
        end
        ...
        function processMACControlElement(obj, macCEInfo, varargin)
            ...
            if(macCEInfo.LCID == 59 || macCEInfo.LCID == 60 || macCEInfo.LCID == 61 || macCEInfo.LCID == 62)
                ...
                obj.BufferStatusULPerUETemp(macCEInfo.RNTI) = sum(obj.BufferStatusUL(macCEInfo.RNTI, :), 2);
            end
        end
    ...
    methods (Access = protected)
        ...
        function downlinkGrants = scheduleDLResourcesSlot(obj, slotNum, lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion)
           %scheduleDLResourcesSlot Schedule DL resources of a slot
            ...
            if obj.SchedulingType == 0  % Slot based scheduling
                % Assignments to span all the symbols in the slot
                downlinkGrants = assignDLResourceTTI(obj, slotNum, firstDLSym, numDLSym, lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion);
            else % Symbol based scheduling
               ...
               % Schedule all DL TTIs of length 'obj.TTIGranularity' in the slot one-by-one
                startSym = firstDLSym;
                for i = 1 : numTTIs
                    TTIDLGrants = assignDLResourceTTI(obj, slotNum, startSym,  obj.TTIGranularity, lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion);
                ...
                if remDLSym >= 2 % PDSCH requires minimum 2 symbols with mapping type B as per TS 38.214 - Table 5.1.2.1-1
                    ...
                    for i = 1:length(smallerTTIs)
                        if(smallerTTIs(i) <= remDLSym)
                            TTIDLGrants = assignDLResourceTTI(obj, slotNum, startSym, smallerTTIs(i), lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion);
                            downlinkGrants(numDLGrants + 1 : numDLGrants + length(TTIDLGrants)) = TTIDLGrants(:);
                            numDLGrants = numDLGrants + length(TTIDLGrants);
        ...
        function dlGrantsTTI = assignDLResourceTTI(obj, slotNum, startSym, numSym, lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion)
            ...
            if any(~rbgAllocationBitmap) && ~isempty(eligibleUEs)
                [~, ~, newTxDLGrants] = scheduleNewTxDL(obj, slotNum, eligibleUEs, startSym, numSym, rbgAllocationBitmap, lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion);
            ...
            for i = 1:length(rbgOccupancyBitmap)
                ...
                if ~rbgOccupancyBitmap(i)
                    ...
                    if selectedUE ~= -1 % If RBG is assigned to any UE
                        ...
                        currentGrant = grant;
                        currentGrant.MCS = obj.MCSForRBGBitmap(rbgMCS(selectedUEIdx, :));
                        updateBufferStatusForGrants(obj, 1, {currentGrant});
        ...
        function [newTxUEs, updatedRBGStatus, DLGrants] = scheduleNewTxDL(obj, scheduledSlot, eligibleUEs, startSym, numSym, rbgOccupancyBitmap, lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion)
            ...
            offsetTime = slotOffset * obj.SlotDuration;

            for i = 1:numel(lchTTL)
                if ~isempty(lchTTL{i})
                    % subtract the offset time up to the scheduled slot
                    % plus an additional slot (since the transmission will
                    % take one slot) from the TTL
                    lchTTL{i} = lchTTL{i} - offsetTime - obj.SlotDuration;
                end
            end
            ...
            for i = 1:length(rbgOccupancyBitmap)
                ...
                if ~rbgOccupancyBitmap(i)
                    RBGIndex = i-1;
                    schedulerInput = createSchedulerInput(obj, obj.DLType, scheduledSlot, RBGEligibleUEs, rank, RBGIndex, startSym, numSym, lchTTL, lchPDB, lchAppID, lchFrameType, lchFramePortion);
                    % Run the scheduling strategy to select a UE for the RBG and appropriate MCS
                    [selectedUE, mcs] = runSchedulingStrategy(obj, schedulerInput);
                    if selectedUE ~= -1 % If RBG is assigned to any UE
                        updatedRBGStatus(i) = 1; % Mark as assigned
                        ...
                        currentGrant = grant;
                        currentGrant.MCS = obj.MCSForRBGBitmap(rbgMCS(selectedUEIdx, :));
                        updateBufferStatusForGrants(obj, 0, {currentGrant});
        ...
        function schedulerInput = createSchedulerInput(obj, linkDir, slotNum, eligibleUEs, selectedRank, rbgIndex, startSym, numSym, varargin)
            ...
            if linkDir % Uplink
                ...
                ueBufferStatus = obj.BufferStatusULPerUETemp;
                ...
                ueBufferStatus = obj.BufferStatusDLPerUETemp;
            ...
            if nargin == 13
                lchTTLFull = varargin{1}; 
                lchPDBFull = varargin{2};
                lchAppIDFull = varargin{3};
                lchFrameTypeFull = varargin{4};
                lchFramePortionFull = varargin{5};

                lchTTL = cell(numel(eligibleUEs),1);
                lchPDB = cell(numel(eligibleUEs),1);
                lchAppID = cell(numel(eligibleUEs),1);
                lchFrameType = cell(numel(eligibleUEs),1);
                lchFramePortion = cell(numel(eligibleUEs),1);
                for i=1:numel(eligibleUEs)
                    u = eligibleUEs(i);
                    for j=1:numel(lchTTLFull(u,:))
                        lchTTL{i} = [lchTTL{i}; lchTTLFull{u,j}];
                        lchPDB{i} = [lchPDB{i}; lchPDBFull{u,j}];
                        lchAppID{i} = [lchAppID{i}; lchAppIDFull{u,j}];
                        lchFrameType{i} = [lchFrameType{i}; lchFrameTypeFull{u,j}];
                        lchFramePortion{i} = [lchFramePortion{i}; lchFramePortionFull{u,j}];
                    end
                    [lchTTL{i}, idx] = sort(lchTTL{i});
                    lchPDB{i} = lchPDB{i}(idx);
                    lchAppID{i} = lchAppID{i}(idx);
                    lchFrameType{i} = lchFrameType{i}(idx);
                    lchFramePortion{i} = lchFramePortion{i}(idx);
                end
                schedulerInput.lchTTL = lchTTL;
                schedulerInput.lchPDB = lchPDB;
                schedulerInput.lchAppID = lchAppID;
                schedulerInput.lchFrameType = lchFrameType;
                schedulerInput.lchFramePortion = lchFramePortion;
            end
        end
        ...
        function updateBufferStatusForGrants(obj, linkType, grants)
            ...
            for grantIdx = 1:length(grants)
                ...
                if linkType % Uplink
                    ...
                    obj.BufferStatusULPerUETemp(rnti) = obj.BufferStatusULPerUE(rnti) - floor(achievedTxBits/8);
                else % Downlink
                    ...
                    obj.BufferStatusDLPerUETemp(rnti) = obj.BufferStatusDLPerUE(rnti) - floor(achievedTxBits/8);
                end
            end
        end
    end
