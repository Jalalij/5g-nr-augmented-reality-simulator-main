classdef hNRSchedulerFrameIntegretyProtection < hNRScheduler
    ...
    methods
        function obj = hNRSchedulerFrameIntegretyProtection(simParameters)
            ...
            for i = 1:length(schedulerInput.eligibleUEs)
                ...
                if(bufferStatus > 0) % Check if UE has any data pending
                    if schedulerInput.LinkDir == 0 % DL
                        ...
                        delaBudgetFactor = zeros(size(schedulerInput.lchTTL{i})); % for every packet in the RLC buffer
                        portionOfTransmittedFrame = zeros(size(schedulerInput.lchTTL{i})); % for every packet in the RLC buffer
                        for j=1:numel(schedulerInput.lchTTL{i})
                            ...
                            delaBudgetFactor(j) = schedulerInput.lchPDB{i}(j)/(schedulerInput.lchTTL{i}(j));
                            portionOfTransmittedFrame(j) = schedulerInput.lchFramePortion{i}(j);
                        end

                        weightingMetric = zeros(size(schedulerInput.lchTTL{i})); % for every packet in the RLC buffer
                        for j=1:numel(schedulerInput.lchTTL{i})
                            if isnan(portionOfTransmittedFrame(j))
                                weightingMetric(j) = 1;
                            else
                                weightingMetric(j) = (exp(portionOfTransmittedFrame(j))-0.99);
                            end
                        end
                        ...
                        pfWeightageFull = (100-priority).*achievableDataRate/pastDataRate.*min(weightingMetric); % minimum because only already transmitted parts shall be taken into account
