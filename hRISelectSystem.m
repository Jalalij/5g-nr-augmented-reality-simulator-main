function [RI,PMISet] = hRISelectSystem(carrier,csirs,reportConfig,H,varargin)
% hRISelectSystem Rank indicator calculation
%   [RI,PMISET] = hRISelectSystem(CARRIER,CSIRS,REPORTCONFIG,H) returns the
...
%
%   [RI,PMISET] = hRISelectSystem(CARRIER,CSIRS,REPORTCONFIG,H,NVAR) specifies
...
%   [RI,PMISet] = hRISelectSystem(carrier,csirs,reportConfig,H,nVar)
...
%   CSIRSIND, and the noise variance NVAR.
    ...
    if ~isscalar(unique(csirs.NumCSIRSPorts))
        error('nr5g:hRISelectSystem:InvalidCSIRSPorts',...
    ...
    if ~all(strcmpi(cdmType,cdmType{1}))
        error('nr5g:hRISelectSystem:InvalidCSIRSCDMTypes',...
    ...
    if ~isfield(reportConfig,'NSizeBWP')
        error('nr5g:hRISelectSystem:NSizeBWPMissing','NSizeBWP field is mandatory.');
    end
    ...
    if ~isfield(reportConfig,'NStartBWP')
        error('nr5g:hRISelectSystem:NStartBWPMissing','NStartBWP field is mandatory.');
    end
    ...
    if nStartBWP < carrier.NStartGrid
        error('nr5g:hRISelectSystem:InvalidNStartBWP',...
    ...
    if (nSizeBWP + nStartBWP)>(carrier.NStartGrid + carrier.NSizeGrid)
        error('nr5g:hRISelectSystem:InvalidBWPLimits',['The sum of starting resource '...
    ...
    if strcmpi(reportConfigOut.PMIMode,'Subband')
        if nSizeBWP >= 24
            if ~isfield(reportConfig,'SubbandSize')
                error('nr5g:hRISelectSystem:SubbandSizeMissing',...
    ...
    if strcmpi(reportConfigOut.PMIMode,'Subband')
        if nSizeBWP >= 24
            ...
            if ~any(NSBPRB == validNSBPRBValues)
                error('nr5g:hRISelectSystem:InvalidSubbandSize',['For the configured BWP size (' num2str(nSizeBWP) ...
