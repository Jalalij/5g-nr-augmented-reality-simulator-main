classdef hNRGNBPhy < hNRPhyInterface
    ...
    properties (Access = private)
        ...
        %PLConfig Information about the path loss model
        % Path loss configuration, specified as an nrPathLossConfig object.
        % This input specifies the scenario characteristics and the path loss model.
        PLConfig

        %ULCarrierFreq Downlink carrier frequency
        ULCarrierFreq = 2.1e9;

        %ShadowFadingLoss Loss caused by shadow fading in dB for each UE
        ShadowFadingLoss

        %LOS Indicator for a LOS connection for each UE
        LOS

        %UEIndoorDistance Distance to the outer wall for each UE
        UEIndoorDistance
    ...
    methods
        function obj = hNRGNBPhy(param)
            ...
            if isfield(param, 'TxAntPanel')
                obj.TxAntPanel = param.TxAntPanel;
            end

            if isfield(param, 'RxAntPanel')
                obj.RxAntPanel = param.TxAntPanel;
            end
            ...
            if isfield(param, 'PLConfig')
                obj.PLConfig = param.PLConfig;
            end

            if isfield(param, 'ShadowFadingLoss')
                obj.ShadowFadingLoss = param.ShadowFadingLoss;
            end

            if isfield(param, 'LOS')
                obj.LOS = param.LOS;
            end

            if isfield(param, 'UEIndoorDistance')
                obj.UEIndoorDistance = param.UEIndoorDistance;
            end

            if isfield(param, 'ULCarrierFreq')
                obj.ULCarrierFreq = param.ULCarrierFreq;
            end
    ...
    methods (Access = private)
        ...
        function rxWaveform = applyChannelModel(obj, pktInfo, currentTime)
            ...
            if isfield(pktInfo, 'RNTI') && ~isempty(obj.PLConfig)
                pathLoss = nrPathLossWithIndoorPropagation(obj.PLConfig,obj.ULCarrierFreq,obj.LOS(pktInfo.RNTI), obj.UEIndoorDistance(pktInfo.RNTI), obj.Node.Position.',pktInfo.Position.');
                rxWaveform = db2mag(-pathLoss-obj.ShadowFadingLoss(pktInfo.RNTI))*waveform;
            else
