function [PL,SF] = nrPathLossWithIndoorPropagation(pathLossConfig,freq,LOS,d2D_in,posBS,posUE)
...
%#codegen    
    ...
    switch scenario
        ...
        case 'RMa'
            ...
            if d2D_in>0
                % penetration and indoor propagation loss
                PL_tw = 5 - 10 * log10(0.3*10^(-(2+0.2*freq/1e9)/10)+0.7*10^(-(5+4*freq/1e9)/10));
                PL = PL + PL_tw + 0.5*d2D_in;
                SF = SF + 4.4;
            end
        ...
        case 'UMa'
            ...
            if freq < 6e9 && d2D_in>0
                % penetration and indoor propagation loss
                PL_tw = 20;
                PL = PL + PL_tw + 0.5*d2D_in;
                SF = 7;
            elseif freq > 6e9 && d2D_in>0
                warning("Indoor PL for FR2 not implemented yet")
            end
